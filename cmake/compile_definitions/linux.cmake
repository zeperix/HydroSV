...
add_compile_definitions(AQUA_PLATFORM="linux")

# AppImage
if(${AQUA_BUILD_APPIMAGE})
    # use relative assets path for AppImage
    string(REPLACE "${CMAKE_INSTALL_PREFIX}" ".${CMAKE_INSTALL_PREFIX}" AQUA_ASSETS_DIR_DEF ${AQUA_ASSETS_DIR})
endif()
...
if(${AQUA_ENABLE_CUDA})
    ...
    elseif(${CUDA_FAIL_ON_MISSING})
        message(FATAL_ERROR
                "CUDA not found.
                If this is intentional, set '-DAQUA_ENABLE_CUDA=OFF' or '-DCUDA_FAIL_ON_MISSING=OFF'"
        )
    endif()
endif()
if(CUDA_FOUND)
    ...
    add_compile_definitions(AQUA_BUILD_CUDA)
endif()
...
if(LIBDRM_FOUND AND LIBCAP_FOUND)
    add_compile_definitions(AQUA_BUILD_DRM)
    ...
    list(APPEND AQUA_DEFINITIONS EGL_NO_X11=1)
endif()
...
# vaapi
if(${AQUA_ENABLE_VAAPI})
    find_package(Libva REQUIRED)
else()
    set(LIBVA_FOUND OFF)
endif()
if(LIBVA_FOUND)
    add_compile_definitions(AQUA_BUILD_VAAPI)
...
# wayland
if(${AQUA_ENABLE_WAYLAND})
    find_package(Wayland REQUIRED)
else()
    set(WAYLAND_FOUND OFF)
endif()
if(WAYLAND_FOUND)
    add_compile_definitions(AQUA_BUILD_WAYLAND)

    if(NOT AQUA_SYSTEM_WAYLAND_PROTOCOLS)
...
if(X11_FOUND)
    add_compile_definitions(AQUA_BUILD_X11)
...
if(${AQUA_ENABLE_TRAY})
    ...
    if(APPINDICATOR_FOUND)
        list(APPEND AQUA_DEFINITIONS TRAY_AYATANA_APPINDICATOR=1)
    else()
        pkg_check_modules(APPINDICATOR appindicator3-0.1)
        if(APPINDICATOR_FOUND)
            list(APPEND AQUA_DEFINITIONS TRAY_LEGACY_APPINDICATOR=1)
        endif ()
    endif()
    ...
    if(NOT APPINDICATOR_FOUND OR NOT LIBNOTIFY_FOUND)
        ...
        list(APPEND AQUA_EXTERNAL_LIBRARIES ${APPINDICATOR_LIBRARIES} ${LIBNOTIFY_LIBRARIES})
    endif()

    # flatpak icons must be prefixed with the app id or they will not be included in the flatpak
    if(${AQUA_BUILD_FLATPAK})
        set(AQUA_TRAY_PREFIX "${PROJECT_FQDN}")
    else()
        set(AQUA_TRAY_PREFIX "apollo")
    endif()
    list(APPEND AQUA_DEFINITIONS AQUA_TRAY_PREFIX="${AQUA_TRAY_PREFIX}")
else()
    set(AQUA_TRAY 0)
...
list(APPEND AQUA_EXTERNAL_LIBRARIES inputtino::libinputtino)
...
if (${AQUA_BUILD_APPIMAGE})
    list(APPEND AQUA_DEFINITIONS AQUA_BUILD_APPIMAGE=1)
endif ()
if (${AQUA_BUILD_FLATPAK})
    list(APPEND AQUA_DEFINITIONS AQUA_BUILD_FLATPAK=1)
endif ()
