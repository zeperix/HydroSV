...
add_executable(sunshine ${AQUA_TARGET_FILES})
foreach(dep ${AQUA_TARGET_DEPENDENCIES})
...
target_link_libraries(sunshine ${AQUA_EXTERNAL_LIBRARIES} ${EXTRA_LIBS})
target_compile_definitions(sunshine PUBLIC ${AQUA_DEFINITIONS})
...
if(CUDA_INHERIT_COMPILE_OPTIONS)
    foreach(flag IN LISTS AQUA_COMPILE_OPTIONS)
        list(APPEND AQUA_COMPILE_OPTIONS_CUDA "$<$<COMPILE_LANGUAGE:CUDA>:--compiler-options=${flag}>")
    endforeach()
endif()

target_compile_options(sunshine PRIVATE $<$<COMPILE_LANGUAGE:CXX>:${AQUA_COMPILE_OPTIONS}>;$<$<COMPILE_LANGUAGE:CUDA>:${AQUA_COMPILE_OPTIONS_CUDA};-std=c++17>)  # cmake-lint: disable=C0301

# Homebrew build fails the vite build if we set these environment variables
if(${AQUA_BUILD_HOMEBREW})
    set(NPM_SOURCE_ASSETS_DIR "")
    set(NPM_ASSETS_DIR "")
    set(NPM_BUILD_HOMEBREW "true")
else()
    set(NPM_SOURCE_ASSETS_DIR ${AQUA_SOURCE_ASSETS_DIR})
...
add_custom_target(web-ui ALL
        ...
        COMMAND "${CMAKE_COMMAND}" -E env "AQUA_BUILD_HOMEBREW=${NPM_BUILD_HOMEBREW}" "AQUA_SOURCE_ASSETS_DIR=${NPM_SOURCE_ASSETS_DIR}" "AQUA_ASSETS_DIR=${NPM_ASSETS_DIR}" "$<$<BOOL:${WIN32}>:cmd;/C>" "${NPM}" run build  # cmake-lint: disable=C0301
